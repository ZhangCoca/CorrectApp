...
export default class WebCanvas extends Component {
    ...
    // 初始化画板
    _init(data){
        this.post({action: '-1',data: this.props.bgurl})
    }

    // 以url的形式添加背景
    _addImageUrl(data){
        this.post({action: 4, data: this.props.bgurl})
    }
        ...
        return (
            <View style={[styles.container, {width:this.state.width, height:this.state.height}]}>
                <WebView
                    ...
                    onLoadEnd={this._addImageUrl.bind(this)}
                />
